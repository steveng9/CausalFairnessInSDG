...
from causal_fairness_sdg.sdg.privbayes import PrivBayes
...
def test_privbayes_baseline_produces_a_valid_bn_over_all_attributes(toy_dataset):
    df, domain, roles = toy_dataset
    result = PrivBayes().fit_generate(
        df, domain, roles, get_fairness_mechanism("none"),
        epsilon=3.0, delta=1e-9, n_synth=200, seed=1,
    )
    # every attribute is either a root (never a child) or placed exactly
    # once as a child of its chosen parent set; graph_edges has one (parent,
    # child) tuple per parent, so a multi-parent child appears more than once
    children = [b for a, b in result.graph_edges]
    assert len(set(children)) == len(set(domain)) - 1  # exactly one root
